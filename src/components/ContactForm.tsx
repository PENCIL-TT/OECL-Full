...
const ContactForm = () => {
  ...
  const [allOffices, setAllOffices] = useState<Record<string, any[]>>({});
  const [pageContent, setPageContent] = useState({
    title: "Get In Touch", subtitle: "Ready to streamline your logistics? Contact us today for a customized solution."
  });
  ...
  const [recipientEmails, setRecipientEmails] = useState(["info@oecl.sg", "oecldm@gmail.com"]);
  ...
  useEffect(() => {
    const fetchOffices = async () => {
      try {
        const { data } = await supabase.from('office_locations').select('*').eq('is_active', true).order('priority', { ascending: true });
        if (data && data.length > 0) {
          const grouped: Record<string, any[]> = {};
          data.forEach(loc => {
            if (!grouped[loc.country]) grouped[loc.country] = [];
            grouped[loc.country].push({
              name: loc.city,
              address: loc.address,
              phone: loc.phone
            });
          });
          setAllOffices(grouped);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        if (data) {
          setPageContent({ title: data.title || "Get In Touch", subtitle: data.subtitle || "Ready to streamline your logistics? Contact us today for a customized solution." });
