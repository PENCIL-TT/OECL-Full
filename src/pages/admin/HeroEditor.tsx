...
const HeroEditor = () => {
  ...
  const fetchSlides = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('hero_slides')
        .select('*')
        .eq('country', 'singapore') // Content is global, so we use 'singapore' as the master source
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const filePath = `singapore/${fileName}`;
      ...
      setUploadForm({ title: "", description: "", priority: 0, file: null });
      fetchSlides();
    } catch (error: any) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editForm.file) {
        ...
        const { error: uploadError } = await supabase.storage
          .from('hero-images')
          .upload(filePath, editForm.file, { upsert: true });
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500"><ImageIcon className="h-12 w-12 mx-auto mb-4 text-gray-300" /><p>No slides found.</p></div>
          ) : (
