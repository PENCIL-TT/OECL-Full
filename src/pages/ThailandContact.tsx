...
const ThailandContact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const emailEndpoints = recipientEmails.length > 0 ? recipientEmails.map(email => `https://formsubmit.co/ajax/${email}`) : ["https://formsubmit.co/ajax/info@oecl.sg"];
    ...
    try {
      ...
      if (allSuccessful) {
        ...
        alert("Submission failed for one or more recipients. Please try again.");
      }
    } catch (err) {
...
export default ThailandContact;
